...
using System;
using SceneConstants = RaytracerPhotonmapping.SceneConstants;
using Vector3D = RaytracerPhotonmapping.math.Vector3D;
using Material = RaytracerPhotonmapping.material.Material;
using Ray = RaytracerPhotonmapping.misc.Ray;
using Intersection = RaytracerPhotonmapping.misc.Intersection;

namespace RaytracerPhotonmapping.objects
{
	public class Sphere:Primitive
	{
		...
		public override bool intersect(Ray ray, Intersection intersection)
		{
			...
			try
			{
				...
				if (l1 >= RaytracerPhotonmapping.SceneConstants.EPSILON && l2 >= RaytracerPhotonmapping.SceneConstants.EPSILON)
				{
					lambda = (l1 < l2)?l1:l2;
				}
				else if (l1 >= RaytracerPhotonmapping.SceneConstants.EPSILON)
				{
					lambda = l1;
				}
				else if (l2 >= RaytracerPhotonmapping.SceneConstants.EPSILON)
				{
					lambda = l2;
				}
				else
