...
using RaytracerPhotonmapping;
using RaytracerPhotonmapping.material;
using RaytracerPhotonmapping.math;
using RaytracerPhotonmapping.misc;
using RaytracerPhotonmapping.objects;
using RaytracerPhotonmapping.photonmap;
...
class RaytracerPhoton
{
	...
	internal RaytracerPhoton()
	{
		new RaytracerPhoton(512, 384, "photontraced.jpg");
	}
	
	internal RaytracerPhoton(int width, int height, System.String outputFilename)
	{
		Sphere sph1, sph2, sph3, sph4, sph5, sph6, sph7, sph8, sph9,ball;
		
	
		//Light light1, light2, light3;
		TriangleAreaLight arealight1, arealight2;
		Vertex roofv1, roofv2, roofv3, roofv4;
		Vertex floorv1, floorv2, floorv3, floorv4;
		Vertex v1, v2, v3, v4;
		Vertex []cube1 = null;
		Vertex []cube2 = null;
		Triangle tri1, tri2;
		...
		Material mirror = new Material(new RGBColor(0.0f, 0.0f, 0.0f), new RGBColor(0.1f, 0.1f, 0.1f), new RGBColor(1.0f, 1.0f, 1.0f), new RGBColor(0.0f, 0.0f, 0.0f), 10.0f, 1.0f);
		...
		glass = new Material(new RGBColor(0.0f, 0.0f, 0.0f), new RGBColor(0.0f, 0.0f, 0.0f), new RGBColor(0.0f, 0.0f, 0.0f), new RGBColor(1.0f, 0.0f, 0.85f), 10.0f, 1.5f);
		Material glasss = new Material(new RGBColor(0.0f, 0.0f, 0.0f), new RGBColor(0.0f, 0.0f, 0.0f), new RGBColor(.2f, 0.2f, 0.2f), new RGBColor(1.0f, 1.0f, 1.0f), 10.0f, 1.52f);
		...
		sph1 = new Sphere(new Vector3D(6, -7, 21), 2);
		ball = new Sphere(new Vector3D(2, -7, 21), 2);
		sph2 = new Sphere(new Vector3D(0 - 5e4 - 12, 0, 0), 5e4);
		//sph2 = new Sphere(new Vector3D(0 - 5e4 - 3, 0, 3), 5e4);
		sph3 = new Sphere(new Vector3D(-3, -3, 21),2);
		...
		floorv1.p = new Vector3D(-20.0, -9, 40);
		floorv2.p = new Vector3D(20.0, -9, 40);
		floorv3.p = new Vector3D(20.0, -9, 0);
		floorv4.p = new Vector3D(-20.0, -9, 0);
		...
		Triangle floortri1 = new Triangle(floorv1, floorv2, floorv3);
		Triangle floortri2 = new Triangle(floorv1, floorv3, floorv4);

		//GenerateCube (ref cube1, ref cubetri1, new Vector3D(-3, -3, 24), new Vector3D (7, 12, 6), 35.0, mirror);
		//	GenerateCube (ref cube2, ref cubetri2, new Vector3D(6, -7, 21), new Vector3D (5, 4, 5), -25.0, glasss);
		...
		sph1.Material = glasss;
		ball.Material = glasss;
		sph3.Material = mirror;
		sph5.Material = mirror;
		sph8.Material = mirror;

		rooftri1.Material = mirror;
		rooftri2.Material = mirror;
		floortri2.Material = cyanWall;
		floortri1.Material = cyanWall;
		sph2.Material = redWall;
		sph4.Material = redWall;
	...
	scene.addPrimitive(ball);
		...
		scene.addPrimitive(floortri1);
		scene.addPrimitive(floortri2);
		...
		photonmap.precomputeRadiance(RaytracerPhotonmapping.SceneConstants.MAX_PHOTON_DIST, RaytracerPhotonmapping.SceneConstants.MAX_PHOTONS);
		after = System.DateTime.Now;

		System.TimeSpan timeDiff = after - before;

		// init irradiance cache
		irradianceCache = new IrradianceCache (RaytracerPhotonmapping.SceneConstants.IRRADIANCE_CACHE_TOLERANCE, RaytracerPhotonmapping.SceneConstants.IRRADIANCE_CACHE_SPACING, scene.getBoundingBox ());
	...
	internal class ScanlineWorkerThread
	{
		...
		private Image			image;
		private RaytracerPhoton	mainInstance;
		private bool			done;
		private RGBColor[]		scanlineRGB;

		public ScanlineWorkerThread (int y, double v, bool doAntialiasing, Image finalImage, RaytracerPhoton main)
		{
			line		 = y;
			this.v		 = v;
	...
	internal virtual RGBColor adaptiveSampling(double u, double stepU, double v, double stepV, RGBColor[] colors, int depth)
	{
		...
		if (((System.Math.Abs(radiance.red() - colors[0].red()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.green() - colors[0].green()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.blue() - colors[0].blue()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF)) && (depth < RaytracerPhotonmapping.SceneConstants.MAX_AA_DEPTH))
		{
		...
		if (((System.Math.Abs(radiance.red() - colors[1].red()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.green() - colors[1].green()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.blue() - colors[1].blue()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF)) && (depth < RaytracerPhotonmapping.SceneConstants.MAX_AA_DEPTH))
		{
		...
		if (((System.Math.Abs(radiance.red() - colors[2].red()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.green() - colors[2].green()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.blue() - colors[2].blue()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF)) && (depth < RaytracerPhotonmapping.SceneConstants.MAX_AA_DEPTH))
		{
		...
		if (((System.Math.Abs(radiance.red() - colors[3].red()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.green() - colors[3].green()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF) || (System.Math.Abs(radiance.blue() - colors[3].blue()) > RaytracerPhotonmapping.SceneConstants.MAX_COLOR_DIFF)) && (depth < RaytracerPhotonmapping.SceneConstants.MAX_AA_DEPTH))
		{
	...
	internal virtual RGBColor shadeRay(double u, double v)
	{
		...
		return traceRay(primaryRay, RaytracerPhotonmapping.SceneConstants.MAX_DEPTH, false);
	}
	...
	RGBColor getIrradiance(Vector3D P, Vector3D N, int depth) 
	{
		...
		int irrM = (int) Math.Max(1, Math.Sqrt(RaytracerPhotonmapping.SceneConstants.IRRADIANCE_SAMPLES / Math.PI));
		...
		if (irr == null) 
		{
			...
			//if ((closest != null) && (closest.intersectionPoint().distanceSqr(P) < RaytracerPhotonmapping.SceneConstants.EPSILON))
			//{
			//	//Trace.WriteLine (u + " " + v);
...
//			scale = 1.0 / (irrM * irrN);
	...
	internal RGBColor causticsRadianceEstimate(Vector3D P, Vector3D N)
	{
		...
		{
			causticsmap.radianceEstimate(radiance, P, N, RaytracerPhotonmapping.SceneConstants.MAX_PHOTON_DIST, RaytracerPhotonmapping.SceneConstants.MAX_PHOTONS);
	...
	internal RGBColor globalRadianceEstimate(Vector3D P, Vector3D N)
	{
		...
		if (photonmap.storedPhotons () > 0)
		{
			...
			photonmap.radianceEstimatePrecomputed(radiance, P, N, RaytracerPhotonmapping.SceneConstants.INITIAL_PHOTON_DIST, RaytracerPhotonmapping.SceneConstants.MAX_PHOTON_DIST_PREC);
		}
	...
	internal virtual RGBColor traceRay(Ray ray, int depth, bool hasBeenDiffuselyReflected)
	{
		...
		if (depth == RaytracerPhotonmapping.SceneConstants.MAX_DEPTH)
		{
	...
	internal virtual void  sampleLights()
	{
		...
		for (System.Collections.IEnumerator elem = lightList.GetEnumerator(); elem.MoveNext(); )
		{
			...
			for (int i = 0; i < light.photons(); i++)
			{
				...
				tracePhoton(sampleRay, power, RaytracerPhotonmapping.SceneConstants.MAX_BOUNCES, false, false);
				...
				if ((i % 100) == 0)
				{
					...
					//for (int t = oldStored; t < causticsmap.storedPhotons(); t++)
					{
						...
						if (p.position != null && p.position.z() > 0)
						{
							...
							if (closest.intersectionPoint().distanceSqr(p.position) < RaytracerPhotonmapping.SceneConstants.EPSILON)
							{
								imagePhotons.setPixel((int) (halfWidth + (2.0 * halfWidth * u / uvCorrection)), (int) (halfHeight - (2.0 * halfHeight * v)), RGBColor.RGBwhite());
	...
	internal virtual void  tracePhoton(Ray ray, RGBColor power, int depth, bool wasSpecularlyReflected, bool hasBeenCausticsAbsorbed)
	{
		...
		{
		//	Trace.WriteLine (closestIntersection.lambda ());w
			return;
		}
		...
		if (diffuseCoef > RaytracerPhotonmapping.SceneConstants.EPSILON)
		{
